<?php
	...
	class PessoaDAO{//Classe DAO (Data Access Object) com os métodos CRUD
		...
		function update($pessoa){//Atualiza os dados no Banco de dados, nas duas tabelas pessoas e telefones
			...
			$telefones = $pessoa->getTelefone();//Configura os parâmetros para montar a query
			...
			try{
				...
				if($status->execute()){//O método execute retorna um boolean true para sucesso e false para fracasso
					...
					if($pessoa->getTelefone() != null){
						...
						//var_dump($telefones[0]);
						foreach($telefones as $telefone){
							//echo "entrei2";
							$query = "INSERT INTO telefones VALUES($id, '$telefone->telefone')";
							//echo $query;
							Conexao::getInstancia()->exec($query);//Altera o telefone sem testar, esta lógica causa um erro de fluxo de dados
