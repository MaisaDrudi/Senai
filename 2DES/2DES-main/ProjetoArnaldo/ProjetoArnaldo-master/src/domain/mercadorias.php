<?php
	...
	class MercadoriasDAO {
		...
		function create($mercadorias) {
			$result = array();
			$nome = $mercadorias-> getNome();
			$valor = $mercadorias-> getValor();
			$quantidade = $mercadorias-> getQuantidade();

			try {
				$query = "INSERT INTO mercadorias VALUES (default, $nome, $valor, $quantidade)";
		...
		function readAll() {
			...
			try {
				...
				$result["erro"] = "Erro na mercadoria.";
			}
		...
		function delete($cod) {
			...
			try {
				...
				if(Connection::getInstance()->exec($query) >= 1){
					$result["msg"] = "Mercadoria foi excluida.";
				}else{
